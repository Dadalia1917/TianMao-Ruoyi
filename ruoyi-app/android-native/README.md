...
> v1.2.1 源码联动：服务端 Agent 已改为显式分派，并固定使用关闭 thinking 的 `qwen3.8-max` 低延迟策略；原生层仍只负责白名单校验、T10S `GenieApi / method=15` 提交、家电/音乐排序和真实回执，不在 Android 端复制规划逻辑。当前已签名真机包仍为 v1.1.2，尚未构建或覆盖安装 v1.2.1。
...
2026 年 8 月 13 日已完成真实重启验证：引导 Activity 启动服务后返回 `com.alibaba.genie.panel`，完整助手 UI 未自动打开；悬浮球持续可见，点击后可恢复 APP。2026.08.17（UTC+8）已把 Agent 自然确认、多方案放松、多动作保序执行和远场采集优化合并为 v1.1.2，覆盖安装到 `192.168.3.234:5555` 后确认 `versionCode=112`、`MainActivity` 和进程正常。当前正式 APK 大小 1,622,763 字节，SHA-256 为 `90B8BC6F90FD91E9E1698541611C19D68D555CC99FF96E2759A8B1454E0F1191`，并已通过 v1/v2 签名校验。
...
3. `index-voice-bridge.js` 读取服务端 `commands` 数组并逐条调用 `window.GenieBridge.sendToGenie(command)`；为适配 T10S 单一会话和音频焦点，客户端内部先提交家电动作、等待该会话结束，再把音乐作为最后一项启动并检测持续播放，未播放时只重试音乐一次。
...
v1.1.2 沿用音乐播放器、空调、风扇等低风险白名单。Agent 可按温度、天气和用户补充要求选择或组合这些设备，但原生层不根据“累了/想放松”自行猜测动作，只执行服务端规划且经用户确认后的明确短命令。诸如先建议音乐、用户再说“可以帮我播放音乐，并且帮我打开空调”，会保留音乐并重新确认；确认后为保证最终音乐不被家电播报抢占，底层先独立提交空调，再提交音乐。只有用户明确说“改成/换成/只要/不需要原方案而是……”才替换原建议。

ADB 实测表明 Provider 接受音乐指令后约 3 秒才进入媒体 `state=3`。旧版先音乐后家电的顺序会让空调播报抢占音频焦点，极端情况下音乐停在暂停态；当前改为家电在前、音乐在后，音乐最终保持 `state=3`。2026.08.17 现场还确认当前天猫账号没有可识别的绑定空调：带或不带“客厅”的空调命令均由天猫云返回“没有找到您要操作的设备”，这属于账号设备绑定/命名问题，不是 APK 丢失命令。

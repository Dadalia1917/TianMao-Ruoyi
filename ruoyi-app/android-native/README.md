...
2026 年 8 月 13 日已完成真实重启验证：引导 Activity 启动服务后返回 `com.alibaba.genie.panel`，完整助手 UI 未自动打开；悬浮球持续可见，点击后可恢复 APP。2026.08.17（UTC+8）已把 Agent 自然确认、多方案放松场景和既有语音优化合并为 v1.1.2，覆盖安装到 `192.168.3.234:5555` 后确认 `versionCode=112`、`MainActivity` 和进程正常。正式 APK 大小 1,621,555 字节，SHA-256 为 `9CE3F9F3993F43FD41E998BD165688BD6ACE7A19E08EBABD926FF33090F8E8BE`，并已通过 v1/v2 签名校验。
...
v1.1.2 沿用音乐播放器、空调、风扇等低风险白名单。Agent 可按温度、天气和用户补充要求选择或组合这些设备，但原生层不根据“累了/想放松”自行猜测动作，只执行服务端规划且经用户确认后的明确短命令。诸如“执行，顺带放一首舒缓的音乐”会以“并且”连接后通过同一个 `GenieApi / method=15` 请求提交。

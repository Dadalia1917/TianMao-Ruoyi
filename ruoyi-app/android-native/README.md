...
> v1.2.1 源码联动：服务端 Agent 已改为显式分派并固定 `qwen3.8-max` thinking 模型，原生层仍只负责白名单校验、T10S `GenieApi / method=15` 提交、家电/音乐排序和真实回执，不在 Android 端复制规划逻辑。当前已签名真机包仍为 v1.1.2，尚未构建或覆盖安装 v1.2.1。

...
- 语音主页建连后显示“等待唤醒”，环境谈话不会进入普通对话；必须先说“天猫管家”。单独呼喊固定回答“我在”，也可在口令后直接附带问题。
- 支持语音结束对话：说“你可以退下了”“我不想跟你说话了”“结束对话”“先这样吧”“再见”等明确结束语，助手回应后自动关闭本轮并回到等待唤醒；下次需要重新说口令。
- 对话结束后客户端拒绝执行延迟到达的 Agent 家居事件，避免已退下状态发生误操作。
...
- 修复 T10S 重启后悬浮入口消失：开机广播通过 1×1 透明引导 Activity 启动前台悬浮服务，完整助手页面保持关闭；点击“智”悬浮球后才进入 APP。
- 最新硬唤醒/主动结束对话正式 APK 已通过 v1/v2 签名校验，SHA-256 为 `652FD50436BAB5FE42A9B0F7740F34008A8E6E1B5AEE40049C4A56E7849B4D5F`。
...
登录后进入语音主页会自动建立连接，但初始状态是“等待唤醒”；H5 首次使用需允许麦克风权限。必须先说“天猫管家”，单独呼喊会收到固定的“我在”，口令后也可直接附带问题。明确说出退下/结束语后本轮关闭，下一轮必须重新唤醒。询问模型身份时会如实回答 Qwen3.5 Omni。

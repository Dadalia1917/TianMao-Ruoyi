...
> 正式 APK：`apk/天猫智家语音助手-v1.1.2.apk`，大小 1,621,555 字节，SHA-256 `9CE3F9F3993F43FD41E998BD165688BD6ACE7A19E08EBABD926FF33090F8E8BE`；v1/v2 签名有效，本地、T10S 与正式云端源码归档一致。

## v1.1.2 更新说明

- Agent 对疲劳、压力和放松不再写死音乐或空调；它结合家庭状态、天气和上一轮选择，在适合当前环境的空调、风扇、音乐播放器中给出有变化但安全的建议。
- 所有设备建议仍需确认。用户可自然表达同意或拒绝，也可说“执行，顺带放一首舒缓的音乐”“换个方案”“改成风扇”“管家，重新开始”或“结束对话”；追加动作会经过同一 Agent 校验后与原方案合并提交。
- 执行、取消或结束后都回到等待“管家”。客户端仍只把命令交给 T10S 内部 `GenieApi / method=15`，不依赖 Home Assistant，也不把“已提交”伪报为实体设备已成功动作。
- v1.1.2 的 H5/App 构建和 Android Release 均通过；T10S `192.168.3.234:5555` 已覆盖安装，未发送真实家电或音乐测试指令。

...
> 发布状态（2026.08.17）：AI-Agent 自然确认、多方案放松场景和既有语音优化已合并为 v1.1.2 正式 APK，完成签名并覆盖安装 T10S；包名 `com.jpx.tmallsmarthome`，`versionCode=112`，应用进程和 `MainActivity` 正常。

> 正式 APK：`apk/天猫智家语音助手-v1.1.2.apk`，大小 1,622,293 字节，SHA-256 `6BC2E4ADBC4422F6D3783E926F29FF3A07F2C5499B3683EF6010F3ECDC09E254`；v1/v2 签名有效，本地与 T10S 已更新。

## v1.1.2 更新说明

- Agent 对疲劳、压力和放松不再写死音乐或空调；它结合家庭状态、天气和上一轮选择，在适合当前环境的空调、风扇、音乐播放器中给出有变化但安全的建议。
- 所有设备建议仍需确认。“需要并且帮我打开空调”“顺带……”“另外再……”和不带替换词的设备补充均追加到原建议；只有“改成/换成/只要/不需要原方案而是……”等明确表达才替换，单独“不需要”则取消。用户也可说“管家，重新开始”或“结束对话”。
- 追加动作经同一 Agent 校验后以有序 `commands` 数组下发；客户端依次调用 `GenieBridge.sendToGenie()`，例如先提交音乐、间隔后再提交空调，避免天猫精灵只解析复合句中的最后一个动作。部分提交失败会返回部分接受状态。
- 执行、取消或结束后都回到等待“管家”。客户端仍只把命令交给 T10S 内部 `GenieApi / method=15`，不依赖 Home Assistant，也不把“已提交”伪报为实体设备已成功动作。
- 非音乐状态使用受限、平滑的远场软件增益；原生桥检测到 T10S 正在播放音乐时将软件增益锁定为 1.0，只保留系统 AEC/降噪。真机遥测已确认音乐状态为 `music=true`、`gain=1.00`。
- v1.1.2 的 H5/App 构建、Android Release 和 FastAPI `129 passed` 均通过；T10S `192.168.3.234:5555` 已覆盖安装并完成音乐、空调两条 Provider 指令的保序提交验证。

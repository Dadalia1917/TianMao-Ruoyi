...
> 正式 APK：`apk/天猫智家语音助手-v1.1.2.apk`，大小 1,622,763 字节，SHA-256 `90B8BC6F90FD91E9E1698541611C19D68D555CC99FF96E2759A8B1454E0F1191`；v1/v2 签名有效，本地与 T10S 已更新。

## v1.2.1 更新说明

- 客户端继续只消费经服务端确认和校验后的有序 `commands`；Agent 不再通过多套分支重复分析同一用户表达。
- “并且/顺带/另外”追加，“只要/改成/换成”替换；服务端已阻止相反开关或相反档位同时下发，客户端继续按家电在前、音乐在后的顺序提交。
- 明确执行、取消、重新唤醒、结束对话和 Provider 回执协议保持不变，不把“已提交”描述成物理设备已经成功动作。

## v1.1.2 更新说明

- Agent 对疲劳、压力和放松不再写死音乐或空调；它结合家庭状态、天气和同账号最近一次选择，在适合当前环境的空调、风扇、音乐播放器中给出有变化但安全的建议。有安全备选时，连续说“我累了”不会重复同一种设备。
- 所有设备建议仍需确认。“需要并且帮我打开空调”“顺带……”“另外再……”和不带替换词的设备补充均追加到原建议；“可以帮我播放音乐，并且帮我打开空调”也会保留音乐并提取空调作为新增项。只有“改成/换成/只要/不需要原方案而是……”等明确表达才替换，单独“不需要”则取消。用户也可说“管家，重新开始”或“结束对话”。
- 追加动作经同一 Agent 校验后以独立 `commands` 数组下发；客户端按 T10S 单一会话/音频焦点特性把家电排在前、音乐排在最后，并在完成后检测音乐是否持续播放。部分提交失败会返回部分接受状态。
...
- v1.1.2 的 H5/App 构建、Android Release 和 FastAPI `133 passed` 均通过；T10S `192.168.3.234:5555` 已覆盖安装并确认音乐稳定为 `state=3`。空调指令已进入天猫内部通道，但当前账号没有可识别的绑定空调，不能把提交回执写成实体空调已打开。
